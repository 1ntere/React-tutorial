...
const MovieRating = () => {
    ...
    useEffect(() => {
        axios.get('https://yts.mx/api/v2/list_movies.json?minimum_rating=9&sort_by=year')
            .then(res => {
                set영화들(res.data.data.movies);//{} 중괄호 들에서 data를 가지고 오겠다.
                        //res.data : 위 url은 대괄호가 없이 중괄호로 시작하므로,
                        //           이럴 때는 임의의 변수명 res을 사용해서 data 대신에 res.data를 통해 {}데이터를 감싸줌
                        //           res = [],
                        //           .data : 데이터를 가져오겠다
                        //           주소값에 key 이름으로 작성된 data에서 movie 라는 키 값 안에 적힌 title과 이미지들을 가져올 것
                        //           [ {data {movie {data} } } ]를 가져올 것
                
                //영화 데이터를 모두 가지고 오면 로딩중을 false로 변경하기
                set로딩(false);
            })
            .catch(err => {
                alert(err + " 발생했습니다.");
