...
기존 HTML에서 주소값을 이동할 때 사용하는 방법

<a href="html파일이름.html">이동하기</a>

a태그로 href를 이용할 경우 html 파일명을 작성해주는 것이기 때문에
React에서는 단순히 a태그와 href만 사용할 수 없음

React는 html파일이 index.html로 1개의 파일 밖에 없기 때문에 단순히 사용할 수 없음

React는 javascript를 이동해서 hrml 파일을 보여주기 때문에

react-touter-dom을 사용
...
2. router를 사용하기 위해 4가지 설정
    BrowserRouter : html에서  url 전체 관리자 역할 (History API 사용, history : 역사, 기록)
    Routes        : 링크 모음을 나타내는 표현 태그
    Route         : 어떤 링크가 어떤 javascript를 표현하는지 표기해주는 태그
    Link          : 사용자가 누르고 이동할 수 있는 링크 생성 a태그에서 href와 똑같은 역할

3. 사용방법
    <BrowserRouter><!-- html에서 이런 링크가 있다 기록을 남김 -->
        <Routes><!-- 링크 표현 모음들(모음집) -->
            <Route path="이동할링크" element={<javascript파일명/>}/>
            <!-- 만약에 / 경로가 Home.js를 바라보길 원한다면 아래와 같이 작성 -->
            <Route path="/" element={<Home/>}/><!-- 맨 위에 Home.js를 import -->
        </Routes>
    </BrowserRouter>
